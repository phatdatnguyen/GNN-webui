...
def on_process_data(working_directory_path, dataset_file_name, graph_directory, datatype, test_ratio, val_ratio, batch_size, random_seed):
    try:
        ...
        return f"<span style='color: green;'>{status}</span>", gr.update(interactive=True), dataset, train_dataloader, val_dataloader, test_dataloader
    except Exception as exc:
        status = f"Error processing data: {exc}"
        return f"<span style='color: red;'>{status}</span>", None, None, None, None, None
...
def train(model, dataloader, device, datatype, optimizer):
    model.train()
    total_loss = 0.0
    num_batches = 0
...
@torch.no_grad()
def validation(model, dataloader, device, datatype):
    model.eval()
    total_loss = 0.0
    num_batches = 0
...
def on_train(model, train_dataloader, val_dataloader, device, datatype, optimizer, scheduler, epochs, trained_epochs, train_losses, val_losses, progress=gr.Progress()):
    # Training loop
    for _ in progress.tqdm(range(trained_epochs, epochs + trained_epochs), total=epochs, desc="Training"):
        train_loss = train(model, train_dataloader, device, datatype, optimizer)
        train_losses.append(train_loss)
        val_loss = validation(model, val_dataloader, device, datatype)
...
@torch.no_grad()
def test(model, dataloader, device, datatype):
    model.eval()
    y_test_list = []
    logits_list = []
    smiles_list = []
...
def on_evaluate(model, dataset, test_dataloader, device, datatype):
    # Get test predictions
    y_test, logits, smiles_list = test(model, test_dataloader, device, datatype)
    probabilities = torch.sigmoid(logits)
    y_test_np = y_test.int().detach().numpy().ravel()
    y_pred_np = torch.round(probabilities).int().detach().numpy().ravel()

    probabilities_np = probabilities.detach().numpy().ravel()
    ...
    eval_df[f"{target_column} (Probability)"] = probabilities_np.tolist()
    ...
    accuracy = round(accuracy_score(y_test_np, y_pred_np), 4)
    precision = round(precision_score(y_test_np, y_pred_np), 4)
    recall = round(recall_score(y_test_np, y_pred_np), 4)
    f1 = round(f1_score(y_test_np, y_pred_np), 4)
    ...
    return f"<span style='color: green;'>{status}</span>", eval_df, metrics_df, cm_figure, roc_auc_figure, gr.update(interactive=True)
...
@torch.no_grad()
def predict(model, dataloader, device, datatype):
    model.eval()
    smiles_list = []
    logits_list = []
...
def on_predict(model, dataset, predict_dataloader, device, datatype):
    # Get predictions
    smiles_list, logits = predict(model, predict_dataloader, device, datatype)
    probabilities = torch.sigmoid(logits)
    y_pred_np = torch.round(probabilities).int().detach().numpy().ravel()
...
def binary_classification_3d_tab_content():
    with gr.Tab("Binary Classification 3D") as binary_classification_3d_tab:
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                with gr.Accordion("Load and process data"):
                    ...
                    with gr.Row():
                        with gr.Column(scale=1):
                            graph_directory_dropdown = gr.Dropdown(label="3D graph directory", choices=[])
                        with gr.Column(scale=1):
    ...
    process_data_button.click(on_process_data, [working_directory_path_state, dataset_file_process_data_dropdown, graph_directory_dropdown, datatype_state, test_size_slider, val_size_slider, batch_size_dropdown, random_seed_slider],
                              [status_html, create_model_button, dataset_state, train_dataloader_state, val_dataloader_state, test_dataloader_state])
    ...
    train_button.click(on_train, [model_state, train_dataloader_state, val_dataloader_state, device_dropdown, datatype_state,
    ...
    evaluate_button.click(on_evaluate, [model_state, dataset_state, test_dataloader_state, device_dropdown, datatype_state],
                                       [status_html, evaluation_df_state, evaluation_metrics_dataframe, confusion_matrix_plot, roc_auc_curve_plot, export_evaluation_button])
    ...
    predict_button.click(on_predict, [model_state, dataset_state, prediction_dataloader_state, device_dropdown, datatype_state],
    ...
    return binary_classification_3d_tab